...
import { ClienteId } from '../../../domain/value-objects/cliente-id.value';
...
export class RegisterClienteHandler
  ...
  async execute(command: RegisterCliente) {
    ...
    let person: Cliente = ClienteFactory.createFrom(personNameResult.value, dniResult.value, auditTrail,command.state);
    ...
    person.changeId(ClienteId.of(convocatoriaId));
